...
void RenderManager::runWaterPipeline() 
{
    ...
    unsigned int texID = getPipeline(WaterPipeline)->generateTextureRenderTarget();

    // "Bind" the newly created texture : all future texture functions will modify this texture
    glBindTexture(GL_TEXTURE_2D, getPipeline(WaterPipeline)->getRenderedTexture(texID));
    ...
    glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, getPipeline(WaterPipeline)->getRenderedTexture(texID), 0);
