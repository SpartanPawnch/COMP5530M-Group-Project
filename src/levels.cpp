...
void loadLevel(const char* path, Scene& scene) {
    ...
    //clear all assets
    audio::audioClearAll();
    //FIXME only clear game textures
    // clearTextures();
        ...
        for (unsigned int i = 0;i < jsonEntities.Size();i++) {
            ...
            for (unsigned int j = 0;j < jsonComponents.Size();j++) {
                ...
                if (strcmp(jsonComponent["type"].GetString(), "TransformComponent") == 0) {
                    ...
                    trComponent->position = glm::vec3(
                        jsonComponent["position"][0].GetFloat(),
                        jsonComponent["position"][1].GetFloat(),
                        jsonComponent["position"][2].GetFloat()
                    );
                    trComponent->rotation = glm::vec4(
                        jsonComponent["rotation"][0].GetFloat(),
                        jsonComponent["rotation"][1].GetFloat(),
                        jsonComponent["rotation"][2].GetFloat(),
                        jsonComponent["rotation"][3].GetFloat()
                    );
                    trComponent->scale = glm::vec3(
                        jsonComponent["scale"][0].GetFloat(),
                        jsonComponent["scale"][1].GetFloat(),
                        jsonComponent["scale"][2].GetFloat()
                    );
                ...
                jsonEntity.components.back()->name = std::string(
                    jsonComponent["name"].GetString());
...
#pragma warning(disable:4996)
    ...
    //encode entities
    {
        ...
        for (unsigned int i = 0;i < scene.entities.size();i++) {
            ...
            jsonEntity.AddMember("name", rapidjson::Value(scene.entities[i].name.c_str(),
                d.GetAllocator()), d.GetAllocator());
            jsonEntity.AddMember("uuid", rapidjson::Value(scene.entities[i].uuid),
                d.GetAllocator());
            ...
            for (unsigned int j = 0;j < scene.entities[i].components.size();j++) {
                ...
                jsonComponent.AddMember("name", rapidjson::Value(scene.entities[i].
                    components[j]->name.c_str(), d.GetAllocator()), d.GetAllocator());
                jsonComponent.AddMember("uuid", rapidjson::Value(scene.entities[i].
                    components[j]->uuid), d.GetAllocator());

                //TransformComponent
                if (auto trComponent = dynamic_cast<TransformComponent*>(scene.entities[i].components[j])) {
                    jsonComponent.AddMember("type", rapidjson::Value("TransformComponent",
                        d.GetAllocator()), d.GetAllocator());
                ...
                //BaseComponent
                    ...
                    jsonComponent.AddMember("type", rapidjson::Value("BaseComponent",
                        d.GetAllocator()), d.GetAllocator());
                }
